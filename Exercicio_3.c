...
//Função criada para exibicao da matriz
...
//Função para analisar se a matriz criada, que possui uma "divisao" por tuplas,
//possui ou nao a caracteristica de ser reflexiva
void Reflexivo(int Mat[][Coluna], int *Boolean){
    ...
    //Verificando se os pares sao reflexivos:
    for(int i = 0; i < Lim; i++){
    ...
    //Verificando se o a quantidade de numeros é igual a quantidade de pares de cada numero presente:
    if(aux == Tam){
...
//Função que verifica se a matriz criada, que possui uma "divisao" por tuplas,
//possui ou nao a caracteristica de ser simetrica
void Simetrico(int Mat[][Coluna]){
    int x, y, aux = 0;

    for(int i = 0; i < Linha; i++){
        x = Mat[i][0]; //Os valores das casas são armazenados nas variaveis.
        y = Mat[i][1];

        for(int j = 0; j < Linha; j++){
            if(Mat[j][0] == y && Mat[j][1] == x){//É entao verificado se, quando invertemos estes valores de posicao,
                aux++;                           //há algum par que seja igual ao que foi invertido, e caso haja, a funcao auxiliar terá a funcao de contador.
                break;                           
            }
        }
    }
...
void Antissimetrico(int Mat[][Coluna]){
    ...
    for(int i = 0; i < Linha; i++){
        ...
        for(int j = 0; j < Linha; j++){
            if(Mat[j][0] == y && Mat[j][1] == x){//É entao verificado se, quando invertemos estes valores de posição,
...
void Transitivo(int Mat[][Coluna]){
    ...
    for(int i = 0; i < Linha; i++){
        x = Mat[i][0];//Os numeros das casas de cada linha são armazenados em variaveis cada vez que o ciclo 'for' é executado.
        y = Mat[i][1];

        for(int j = 0; j < Linha; j++){
            if(Mat[j][0] == y){//Caso o primeiro numero de um par de uma linha qualquer seja igual a y,
                z = Mat[j][1];//então o segundo numero deste par desta linha qualquer é armazenado na variavel z.
                for(int k = 0; k < Linha; k++){
                    if(Mat[k][0] == x && Mat[k][1] == z){//Caso haja um par de numeros em uma linha qualquer que possua x como o primeiro numero e z como o segundo,
                        break;                           //temos então uma relacao transitiva.
                    }
